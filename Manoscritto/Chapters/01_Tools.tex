...
\begin{Def}
  ...
  \[
  \|\A\| = \sqrt{\sum_{i_1 = 1}^{I_1}\sum_{i_2 = 1}^{I_2}\cdots\sum_{i_d = 1}^{I_d}\A^2(i_1 i_2 \cdots i_d)}
  \]
\end{Def}
...
\e{Kronecker} and \e{Hadamard} matrix products are important basic notations and can be straighforwardly generalized to tensors. We now briefly define them as reference for the following chapters.
...
We can define a tensor-by-matrix product in several ways. In this work we will always refer to \e{mode-k contraction} or \e{mode-k multiplication}.\todo{Forse superflua?}
