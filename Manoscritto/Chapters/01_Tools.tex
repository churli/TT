...
\begin{Def}
  The higher-order analogue of matrix rows and columns are called \emph{fibers}. The \emph{i-th fiber} of a tensor \ttensor{X} is the vector of elements given by fixing all the indices but the i-th.
\end{Def}
...
\begin{Def}
  An N-way tensor \ttensor{X} is \emph{rank one} if it can be written as the outer product of N vectors:
  \[
