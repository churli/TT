...
\begin{Teo} \label{teo:ttsvd}
  ...
  \begin{proof}
    ...
    $A_1$ the first unfolding matrix of \A. By the theorem's hypoteses $rk(A_1)~=~r_1$, therefore $A_1$ admits a dyadic (skeleton) decomposition%\todo{21 Jan 2016: Aspetta aspetta, ma da dove viene questa roba? Ma non doveva esserci la SVD qui?}
    \begin{equation*}
      A_1 = UV^\top
    \end{equation*}
    ...
    We can now continue similarly by considering $V'$ as a $(d-2)$-dimensional tensor and so on, yielding all the cores up to $G_d(\alpha_{d-1},i_d)$ and therefore the entire TT-decomposition.%\todo{26 Apr 2016: Ci sto mettendo le mani, speriamo di finire a questo giro.}

    Now we have to prove the assumption $rk(V_k) \leq r_k$. Indeed, the $k$-th TT-rank of \A is equal to $r_k$, therefore \A can be represented as\todo{Skeleton decomp.?}
    \begin{equation*}
...
\begin{Teo} \label{teo:ttsvd_approx}
  ...
  \begin{proof}
    ...
    From the properties of the SVD we have $U_1^\top E_1 = 0$ \todo{Da approfindire/chiarire} , thus
    \begin{equation*}
      \begin{split}
...
\begin{Cor}
  ...
  \begin{proof}
    ...
    \end{equation*}
    Moreover, $||\A - \B^{min}||_F = \eps$, \todo{Check.} so $\B^{min}$ is the minimum.
...
\begin{Cor}
  From \eqref{teo:ttsvd_approx} it also follows that if the SVD \todo{Aggiungere in chapter 1 cos'è la SVD, definiz e proprietà} of the unfoldings are truncated at $\delta$, the approximation error of the whole TT will be $\delta \sqrt{d-1}$. So to obtain any prescribed accuracy $\eps$ on the TT whe have to set the treshold to $\delta = \frac{\eps}{d-1}$.
\end{Cor}

\begin{lstlisting}[title=TT-SVD compression algorithm]
  ttsvd($A$,$\eps$):
...
\begin{equation*}
  P = X \Sigma Y^\top
\end{equation*}
where $\Sigma$ is an $\hat{r}\times\hat{r}$ diagonal matrix, $X$ and $Y$ are $r \times \hat{r}$ matrices with orthonormal columns and $\hat{r}$ is the $\eps$-rank of $D$ (which is equal to the $\eps$-rank of $A_1$)\todo{Dimostrazione?}.
...
\todo{Aggiungere algoritmo rounding qui}
