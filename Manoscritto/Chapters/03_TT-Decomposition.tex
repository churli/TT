...
It is thus interesting considering the case of having a tensor in an already structured format. More specifically, we can assume having a tensor in the TT format already, but stored with suboptimal ranks $r_k$. This is an important case since, as will be shown later, such tensors can result from performing many basic linear algebra operations in TT format. In this case ranks $r_k$ have to be reduced while mantaining the prescribed accuracy: what we need here is a \e{rounding} algorithm, to recompress the tensor to optimal ranks while keeping the TT format.
...
\end{equation*}
and assume its ranks $r_k$ are \e{increased} in respect to the optimal ranks $r_k^\prime$. We want now estimate these optimal ranks $r_k^\prime \le r_k$ while keeping the prescribed accuracy $\eps$.

First, we try to compute $r_1^\prime$ and reduce our TT representation to this rank value. The corresponding unfolding matrix $A_1$ can be written as
