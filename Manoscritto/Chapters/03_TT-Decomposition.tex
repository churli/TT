...
Using the above lemma we can now design a fast algorithm for a structured QR decomposition of the matrix V, assuming it is in the TT format as defined in \ref{def:UV}. The idea is to perform a single right-to-left othogonalization sweep through all the cores
