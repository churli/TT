...
The outcoming algorithm is called \e{TT-cross} and it is shown \todo{Dimostrare!} that, if TT ranks are bounded by $r$, then only $\ord(dnr^2)$ carefully-chosen elements are required in order to completely recover the tensor.

\section{Skeleton decomposition for matrices}
First of all, we need to introduce the skeleton decomposition for the exact matrix case. We are not going to give any proof of its properties here and the interested reader can find them on [???] \todo{Aggiungere ref a \emph{Grantmacher-Theory of Matrices}}.
...
We can now notice that, if $\hat{A}_1$ is of maximal volume in $C$, then $G_1$ has elements not higher than 1 in modulus \todo{Add proof}. This property on the elements of $G_1$ holds when $\hat{A}_1$ is the output of \emph{maxvol} \todo{add refs here, can be found at page 9 of tt-cross paper}.
...
Moving to a numerically viable algorithm requires a careful selection of \emph{sufficiently good} submatrices among those that are non-singular.\todo{Here report a hint about approximate skeleton and errors involved}

\section{A TT-cross algorithm}
TODO!


%%EOF\todo{Continua... a pagina 12 dell'articolo}
%EOF
